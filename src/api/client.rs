...
impl CloudflareClient {
    ...
    pub async fn get<T: DeserializeOwned>(&self, path: &str) -> Result<ApiResponse<T>> {
        ...
        if !api_response.success {
            let errors: Vec<String> = api_response.errors
                .iter()
    ...
    pub async fn post<T: DeserializeOwned>(&self, path: &str, body: Value) -> Result<ApiResponse<T>> {
        let url = format!("{}{}", CF_API_BASE, path);
    ...
    pub async fn patch<T: DeserializeOwned>(&self, path: &str, body: Value) -> Result<ApiResponse<T>> {
        let url = format!("{}{}", CF_API_BASE, path);
        debug!("PATCH {} with body: {}", url, body);
        
        let response = self.build_request(Method::PATCH, &url)
            .json(&body)
            .send()
            .await?;
    ...
    pub async fn put<T: DeserializeOwned>(&self, path: &str, body: Value) -> Result<ApiResponse<T>> {
        let url = format!("{}{}", CF_API_BASE, path);
    ...
    pub async fn graphql(&self, query: &str, variables: Option<Value>) -> Result<Value> {
        ...
        let response = self.build_request(Method::POST, CF_GRAPHQL_URL)
            .json(&body)
            .send()
            .await?;
