...
fn parse_since(since: &str) -> String {
    ...
    (Utc::now() - duration).format("%Y-%m-%dT%H:%M:%SZ").to_string()
}
...
fn get_analytics_data(response: &serde_json::Value) -> Option<&Vec<serde_json::Value>> {
    response
        ...
        .and_then(|a| a.first())
        .and_then(|z| z.get("httpRequestsAdaptiveGroups").or_else(|| z.get("firewallEventsAdaptiveGroups")))
...
fn print_firewall_response(response: &serde_json::Value) -> Result<()> {
        ...
        .and_then(|g| g.as_array())
    {
        ...
        for group in groups {
            ...
            let action = dims.and_then(|d| d.get("action")).and_then(|v| v.as_str()).unwrap_or("-");
            let ip = dims.and_then(|d| d.get("clientIP")).and_then(|v| v.as_str()).unwrap_or("-");
            let country = dims.and_then(|d| d.get("clientCountryName")).and_then(|v| v.as_str()).unwrap_or("-");
            let path = dims.and_then(|d| d.get("clientRequestPath")).and_then(|v| v.as_str()).unwrap_or("-");
...
fn print_hourly_response(response: &serde_json::Value) -> Result<()> {
        ...
        .and_then(|g| g.as_array())
    {
        ...
        for group in groups {
            ...
            let requests = sum.and_then(|s| s.get("requests")).and_then(|v| v.as_u64()).unwrap_or(0);
            let bytes = sum.and_then(|s| s.get("bytes")).and_then(|v| v.as_u64()).unwrap_or(0);
            let cached = sum.and_then(|s| s.get("cachedBytes")).and_then(|v| v.as_u64()).unwrap_or(0);
            let threats = sum.and_then(|s| s.get("threats")).and_then(|v| v.as_u64()).unwrap_or(0);
