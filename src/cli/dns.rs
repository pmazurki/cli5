...
pub async fn execute(config: &Config, args: DnsArgs) -> Result<()> {
    ...
    match args.command {
        ...
        DnsCommand::Get { id } => {
            let response = client.get_raw(&format!("/zones/{}/dns_records/{}", zone_id, id)).await?;
        ...
        DnsCommand::Add { name, record_type, content, proxied, ttl, priority } => {
            let mut body = json!({
            ...
            let response = client.post_raw(&format!("/zones/{}/dns_records", zone_id), body).await?;
        ...
        DnsCommand::Update { id, name, content, proxied, ttl } => {
            // First get current record
            let current = client.get_raw(&format!("/zones/{}/dns_records/{}", zone_id, id)).await?;
            let current = current.get("result").ok_or_else(|| anyhow::anyhow!("Record not found"))?;
            ...
            let response = client.put_raw(&format!("/zones/{}/dns_records/{}", zone_id, id), body).await?;
        ...
        DnsCommand::Delete { id, yes } => {
            ...
            client.delete_raw(&format!("/zones/{}/dns_records/{}", zone_id, id)).await?;
            output::success(&format!("Deleted DNS record: {}", id));
        }
        
        DnsCommand::Export => {
            let response = client.get_raw(&format!("/zones/{}/dns_records", zone_id)).await?;
...
impl CloudflareClient {
    pub async fn post_raw(&self, path: &str, body: serde_json::Value) -> Result<serde_json::Value, anyhow::Error> {
        use crate::api::response::ApiResponse;
        let response: ApiResponse<serde_json::Value> = self.post(path, body).await?;
        Ok(serde_json::json!({"result": response.result}))
    }
    
    pub async fn put_raw(&self, path: &str, body: serde_json::Value) -> Result<serde_json::Value, anyhow::Error> {
        use crate::api::response::ApiResponse;
