...
pub async fn execute(config: &Config, args: SettingsArgs) -> Result<()> {
    ...
    match args.command {
        SettingsCommand::List => {
            let response = client.get_raw(&format!("/zones/{}/settings", zone_id)).await?;
            ...
            if let Some(settings) = response.get("result").and_then(|r| r.as_array()) {
                for setting in settings {
                    ...
                    let editable = setting.get("editable").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        SettingsCommand::Get { name } => {
            let response = client.get_raw(&format!("/zones/{}/settings/{}", zone_id, name)).await?;
        ...
        SettingsCommand::Ssl { mode } => {
            ...
            if !valid.contains(&mode.as_str()) {
                output::error(&format!("Invalid SSL mode. Valid options: {}", valid.join(", ")));
                return Ok(());
            }
            
            let body = json!({"value": mode});
            client.patch_raw(&format!("/zones/{}/settings/ssl", zone_id), body).await?;
        ...
        SettingsCommand::Https { value } => {
            ...
            client.patch_raw(&format!("/zones/{}/settings/always_use_https", zone_id), body).await?;
            output::success(&format!("Always Use HTTPS: {}", val));
        }
        
        SettingsCommand::Security { level } => {
            let valid = ["off", "essentially_off", "low", "medium", "high", "under_attack"];
            if !valid.contains(&level.as_str()) {
                output::error(&format!("Invalid security level. Valid options: {}", valid.join(", ")));
                return Ok(());
            }
            
            let body = json!({"value": level});
            client.patch_raw(&format!("/zones/{}/settings/security_level", zone_id), body).await?;
        ...
        SettingsCommand::CacheLevel { level } => {
            ...
            if !valid.contains(&level.as_str()) {
                output::error(&format!("Invalid cache level. Valid options: {}", valid.join(", ")));
                return Ok(());
            }
            
            let body = json!({"value": level});
            client.patch_raw(&format!("/zones/{}/settings/cache_level", zone_id), body).await?;
            output::success(&format!("Cache level set to: {}", level));
        }
        
        SettingsCommand::BrowserCacheTtl { seconds } => {
            let body = json!({"value": seconds});
            client.patch_raw(&format!("/zones/{}/settings/browser_cache_ttl", zone_id), body).await?;
        ...
        SettingsCommand::Minify { css, html, js } => {
            ...
            let current = client.get_raw(&format!("/zones/{}/settings/minify", zone_id)).await?;
            ...
            client.patch_raw(&format!("/zones/{}/settings/minify", zone_id), body).await?;
        ...
        SettingsCommand::Set { name, value } => {
            ...
            client.patch_raw(&format!("/zones/{}/settings/{}", zone_id, name), body).await?;
...
impl CloudflareClient {
    pub async fn patch_raw(&self, path: &str, body: serde_json::Value) -> Result<serde_json::Value, anyhow::Error> {
